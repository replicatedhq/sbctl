// The describe command uses client-side formatting.  Unfortunately kubectl does not export
// the necessary functions, so this is copied and modified from
// https://github.com/kubernetes/kubectl/blob/e18be6f487b9b8192fdc0df9aed507a18cdba6d7/pkg/describe/describe.go
...
func PrintNamespacedDescribe(rootDir string, resource string, resourceName string, namespace string) error {
	if namespace == "" {
		namespace = "default"
	}
	...
	err = printDescribe(data, resourceName)
	if err != nil {
...
func PrintClusterDescribe(rootDir string, resource string, resourceName string) error {
...
func printDescribe(data []byte, resourceName string) error {
	...
	case *corev1.PodList:
		for _, pod := range o.Items {
			if pod.Name == resourceName {
				var events *corev1.EventList // TODO
				outputString, formatError = describePod(&pod, events)
				break
			}
		}
	case *corev1.Pod:
		var events *corev1.EventList // TODO
		outputString, formatError = describePod(o, events)
	case *appsv1.ReplicaSetList:
		for _, rs := range o.Items {
			if rs.Name == resourceName {
				var events *corev1.EventList // TODO
				...
				// }
				outputString, formatError = describeReplicaSet(&rs, events, 0, 0, 0, 0, nil)
				break
			}
		}
	case *appsv1.ReplicaSet:
		...
		// 	events, _ = searchEvents(d.CoreV1(), rs, describerSettings.ChunkSize)
		// }
		outputString, formatError = describeReplicaSet(o, events, 0, 0, 0, 0, nil)
	case *corev1.NamespaceList:
		for _, ns := range o.Items {
			if ns.Name == resourceName {
				resourceQuotaList := &corev1.ResourceQuotaList{} // TODO
				limitRangeList := &corev1.LimitRangeList{}       // TODO
				outputString, formatError = describeNamespace(&ns, resourceQuotaList, limitRangeList)
				break
			}
		}
	case *corev1.Namespace:
		resourceQuotaList := &corev1.ResourceQuotaList{} // TODO
		limitRangeList := &corev1.LimitRangeList{}       // TODO
		outputString, formatError = describeNamespace(o, resourceQuotaList, limitRangeList)
	default:
		return errors.Errorf("cannot describe %T", decoded)
	}

	if formatError != nil {
		return errors.Wrapf(formatError, "failed to format obect %s", resourceName)
	}
